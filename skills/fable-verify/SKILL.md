...
- **Discharge residuals:** when this evidence settles an entry in `.fable/residuals.md`, **delete the line** (append its resolution to `.fable/claims-log` if it became a `Verified:`) and announce it — the counters treat any remaining `Assumed:`/`PROVISIONAL` line as open.
