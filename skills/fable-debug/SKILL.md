...
8. **Root-cause the escape.** Always the second question: why did the existing checks miss this? Mint the runnable rule — the test/assert/check that would have caught it — and log the gotcha (`trap → cause → rule`) to `.fable/project.md`. If a task file is open in `.fable/tasks/`, record the decision trail there.
