...
- **Separate verified from assumed, out loud.** "I verified X by running Y" vs "I'm assuming Z because I couldn't check it." Never let a sentence from the second bucket read like the first.
...
If `.fable/project.md` exists, **compact it as part of shipping**: fold in the durable facts and **gotchas** you confirmed this task (`Gotcha: <trap> → Cause → Rule`), dedup, retire entries that went obsolete, promote recurring ones, keep it to ~a page, and announce what changed. Log gotchas liberally — over-capture beats missing one.
