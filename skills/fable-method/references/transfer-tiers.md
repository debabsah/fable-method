...
*Which* unknown is load-bearing; *which* rule to try next when a diff won't close; *when* to reframe an ask upward; whether a green is suspect *this time*. A capable model (the target here, Opus 4.8) supplies much of this; a weaker one needs the Tier-2 scaffolding to compensate. In the source session a sharp operator also supplied part of the judgment at key moments.

## The honest caveat
The distillation is **n = 1** — one operator, one project, one model version, over days. Treat "The Fable Way" as validated *for that setting*; re-check before assuming it generalizes everywhere. The plugin encodes the transferable part; it does not turn a model into Fable.
